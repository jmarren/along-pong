...
static void write(uv_stream_t* stream, char* type, char* content) {
	uv_buf_t wrbuf;
	...
	uv_write(req, stream, &wrbuf, 1, cb_free_write);

}

static void handle_tcp_read(uv_stream_t* client, ssize_t nread, const uv_buf_t *buf) {
    if (nread < 0) {
        if (nread != UV_EOF) {
...
static void handle_connection(uv_stream_t *stream, int status) {
...
static void init(server_t* server_ref) {
    server = server_ref;
    ...
    err = uv_listen((uv_stream_t*) tcp_server, BACKLOG, handle_connection);
